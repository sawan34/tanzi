...
function woo_add_payment_method_to_admin_new_order( $order, $is_admin_email ) { 
	...
	echo '<p><strong>Payment Method:</strong> ' . $order->cod . '</p>'; 
				...
				add_option( 'alterinventory_error_message', '<h1 style="color:#F00">Please Login ! enter your user name and password!</h1>' );
	    ...
	    do_action( 'woocommerce_update_options' );
				...
				if ( $this->alterinventory_enabled ) {
					function get_inventory() {
						...
						$out = get_option('<h1 style="color:#F00">alterinventory_error_message</h1>', '<h1 style="color:#F00">Please Log In ! enter your Username and Password!</h1>');
                            ...
                            <table width="100%" style="border: 1px solid #000; width: 100%; margin-bottom: 50px" cellspacing="0" cellpadding="2" >
								<thead>
									...
									<!--	<th scope="col" style="border: 1px solid #000; padding: 6px;"><?php _e('SKU', 'woothemes'); ?></th>	-->
									  ...
									  <th scope="col" style="border: 1px solid #000; padding: 6px;"><?php _e('Price', 'woothemes'); ?></th>
...
<td class="order-number" style="padding: 0 5px;border: 1px solid #000;">
	<input type="text" id="weight_<?php _e(get_the_id()); ?>" class="pro_weight" value="<?php echo $product->get_attribute( 'pa_zweight' ); ?>" /></td>
...
<?php
  ...
  if($sale_price=='' || !$sale_price){
    $pro_price = get_post_meta( get_the_ID(), '_regular_price', true);
  }else{
